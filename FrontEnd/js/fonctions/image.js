...
export async function removePicture(e) {
  ...
  if (confirmDelete === true) {
    ...
    const pictureOnIndex = document.querySelector(
      `figure[data-id="${idDelete}"]`
    );
...
export async function addPicture(event) {
  ...
  if (r === 401) {
    addErrorMessage(
      "Vous n'êtes pas autorisé(e) ou token trop vieux !",
      "#myModalGallery .js-picture"
    );
    return;
  }
...
export function templateSubmit__Origin() {
  document
    .querySelector("#pictureSubmit")
    .classList.add("modal__btn--noComplet");
  document.querySelector("#pictureSubmit").setAttribute("disabled", "");
}
export function templateSubmit__Valid() {
  document
    .querySelector("#pictureSubmit")
    .classList.remove("modal__btn--noComplet");
...
export function templateLabelBtn__Origin() {
  ...
  document
    .querySelector("#pictureView")
    .classList.remove("addPicture__logo--full");
  document
    .querySelector("#pictureView")
    .classList.add("addPicture__logo--empty");
  document.querySelector("#pictureForm .addPicture__btn").style =
    "display:null;";
  document.querySelector("#pictureForm .contener-photo p").style =
    "display:block;";
}
export function templateLabelBtn__Select(imageSelect) {
  ...
  document
    .querySelector("#pictureView")
    .classList.remove("addPicture__logo--empty");
  document
    .querySelector("#pictureView")
    .classList.add("addPicture__logo--full");
  document.querySelector("#pictureForm .addPicture__btn").style =
    "display:none;";
  document.querySelector("#pictureForm .contener-photo  p").style =
    "display:none;";
